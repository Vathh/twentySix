...
@section('content')
    <div class="container mx-auto py-8 max-w-4xl text-text-primary">
        ...
        @else
            @php
                $legsDetailList = $legsDetail->values();
                $defaultLegTab = 0;
                foreach ($legsDetailList as $idx => $block) {
            ...
            <div
                class="mb-8"
                x-data="{ activeLeg: {{ $defaultLegTab }} }"
            >
                <div class="flex flex-wrap gap-2 mb-4" role="tablist" aria-label="Legi">
                    @foreach($legsDetailList as $idx => $block)
                        <button
                            type="button"
                            role="tab"
                            class="px-3 py-1.5 rounded-lg text-sm font-semibold border transition"
                            x-bind:class="activeLeg === {{ $idx }}
                                ? 'bg-light-orange/20 text-light-orange border-light-orange/50'
                                : 'bg-darker-bg text-text-muted border-border hover:border-light-green/40 hover:text-light-white'"
                            x-on:click="activeLeg = {{ $idx }}"
                            x-bind:aria-selected="activeLeg === {{ $idx }}"
                        >
