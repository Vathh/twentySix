    ...
    <div class="flex min-h-screen bg-dark-bg text-light-white">
        ...
        <aside class="w-72 backdrop-blur bg-white/5 border-r border-white/10 p-6 flex flex-col">
            ...
            <nav class="flex flex-col space-y-3">
                <a href="{{ route('seasons.create') }}?leagueId={{ $season->id }}"
                   class="flex items-center gap-3 bg-white/10 hover:bg-white/15 px-4 py-3 rounded-lg transition">
                    ➕ Dodaj sezon
                </a>
                <a href="{{ route('leagues.admins', $season->id) }}"
                   class="flex items-center gap-3 bg-white/10 hover:bg-white/15 px-4 py-3 rounded-lg transition">
                    ‍💼 Administratorzy
                </a>
                <a href="{{ route('leagues.edit', ['league' => $season->id]) }}"
                   class="flex items-center gap-3 bg-white/10 hover:bg-white/15 px-4 py-3 rounded-lg transition">
                    ✏️ Edytuj ligę
                </a>
                <a href="{{ route('leagues.relatedUsers', $season->id) }}"
                   class="flex items-center gap-3 bg-white/10 hover:bg-white/15 px-4 py-3 rounded-lg transition">
        ...
        <div class="flex-1 p-10 flex justify-center">
            <div class="max-w-3xl w-full">
                ...
                <div class="bg-white/5 border border-white/10 p-6 rounded-xl shadow-lg backdrop-blur">
                    <p class="mb-2"><span
                            class="text-light-green font-semibold">Opis:</span> {{ $season->description }}</p>
                    <p class="mb-2"><span
                            class="text-light-green font-semibold">Data utworzenia:</span> {{ $season->createdAtDate() }}
                    </p>
                    <p class="mb-2"><span
                            class="text-light-green font-semibold">Liczba sezonów:</span> {{ count($season->seasons) }}
                    </p>
                    <p><span
