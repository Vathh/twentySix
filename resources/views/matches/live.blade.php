...
@section('content')
    ...
    >
        <div class="flex flex-wrap items-center gap-3 mb-4">
            ...
            <a href="{{ route('matches.show', ['type' => $kind, 'id' => $matchId]) }}" class="text-light-orange hover:underline text-sm">
                Szczegóły meczu
            </a>
        </div>
        ...
        <h1 class="text-xl font-bold text-light-green mb-6 text-center">
            <span x-text="player1?.name ?? '{{ $player1->name }}'"></span>
            <span class="text-text-muted font-normal mx-2">vs</span>
            <span x-text="player2?.name ?? '{{ $player2->name }}'"></span>
        </h1>
        ...
        <div x-show="tab === 'counter'" x-cloak>
            ...
            <div class="grid grid-cols-3 items-center gap-4 mb-6 text-center">
                <div>
                    <p class="text-sm text-text-muted mb-1" x-text="player1?.name"></p>
                    <p class="text-4xl font-bold text-light-green" x-text="player1?.legsWon ?? 0"></p>
                </div>
                <div>
                    <p class="text-xs text-light-orange uppercase tracking-wide mb-1">Legi</p>
                    <p class="text-lg text-text-muted" x-text="currentLegLabel"></p>
                    <p class="text-xs text-text-muted mt-1">
                        do <span x-text="state?.match?.legsToWin ?? 2"></span> wygranych
                    </p>
                </div>
                <div>
                    <p class="text-sm text-text-muted mb-1" x-text="player2?.name"></p>
                    <p class="text-4xl font-bold text-light-green" x-text="player2?.legsWon ?? 0"></p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4 mb-6">
                <div class="bg-lighter-bg rounded-xl p-6 text-center border border-border">
                    ...
                    <p class="text-5xl font-bold text-light-white tabular-nums" x-text="player1?.remaining ?? '—'"></p>
                </div>
                <div class="bg-lighter-bg rounded-xl p-6 text-center border border-border">
                    <p class="text-xs text-text-muted mb-2">Pozostało</p>
                    <p class="text-5xl font-bold text-light-white tabular-nums" x-text="player2?.remaining ?? '—'"></p>
                </div>
            </div>
            ...
            <div class="bg-darker-bg rounded-lg border border-border p-4">
                ...
                <div class="overflow-x-auto" x-show="visits.length > 0">
                    <table class="w-full text-sm text-light-white">
                        <thead>
                            <tr class="text-text-muted text-xs border-b border-border">
                                ...
                                <th class="text-left py-2 pr-2">Gracz</th>
                                <th class="text-center py-2 px-2">Pkt</th>
                                <th class="text-center py-2 px-2">Przed</th>
                                <th class="text-center py-2 px-2">Po</th>
                            </tr>
                        </thead>
                        <tbody>
                            <template x-for="visit in visits" x-bind:key="visit.id">
                                <tr class="border-b border-border/40" x-bind:class="visit.bust ? 'opacity-60' : ''">
                                    <td class="py-2 pr-2" x-text="visit.visitNumber"></td>
                                    <td class="py-2 pr-2" x-text="playerName(visit.playerId)"></td>
                                    <td class="text-center py-2 px-2" x-text="visit.bust ? 'Bust' : visit.score"></td>
                                    <td class="text-center py-2 px-2" x-text="visit.remainingBefore"></td>
        ...
        <div x-show="tab === 'stats'" x-cloak>
            <div class="bg-lighter-bg rounded-lg border border-border overflow-hidden">
                ...
                <template x-for="row in [
                    ...
                    { label: 'Średnia (mecz)', key: 'matchAverage' },
...
@section('scripts')
    @vite('resources/js/match-live.js')
@endsection
